...
export default function LiveClient({ gigsData }) {
  // Evaluated in the browser on every page load — always reflects today's real date
          ...
          {upcomingShows.length > 0 ? (
            <div className="shows-list">
              {upcomingShows.map((gig, i) => (
                <ShowRow key={gig.id} gig={gig} index={i} today={today} />
              ))}
            </div>
          ) : (
            <>
